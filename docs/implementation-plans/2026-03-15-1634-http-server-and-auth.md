...
- [x] Plugin loads in Obsidian — HTTP server starts without errors on configured port
- [x] Plugin unloads cleanly — HTTP server stops, all sessions closed, port released, no orphaned listeners
- [x] API key auto-generated on first load, persisted in `SecretStorage`
- [x] API key visible in settings tab, copyable
- [x] API key regeneration works (confirmation dialog, new key saved, "How to Connect" updated)
- [x] Port configurable in settings (confirmation dialog, server restarts, "How to Connect" updated)
- [x] "How to Connect" section shows correct `mcp.json` snippet and `claude mcp add` command
- [x] Requests without valid Bearer token rejected with 401
- [x] POST /mcp with initialize request creates new session and connects transport to McpServer
- [x] POST/GET/DELETE on `/mcp` with valid session ID routed to existing transport
- [x] Requests with invalid or missing session ID (non-initialize) return 400
- [x] Unknown routes return 404
- [x] McpServer created with zero tools — MCP initialization handshake succeeds
- [x] All tests pass
...
| 2026-03-15 | Task 5 | plugin/src/settings.ts, plugin/src/settings.test.ts, plugin/src/**mocks**/obsidian.ts, plugin/package.json             | Created settings tab and tests. Added `jsdom` dev dependency for DOM-based tests. Added `createObsidianEl` helper to mock for `createEl`/`empty()` augmented methods. `Setting` mock now appends to containerEl with working `setName`/`setDesc`/`setValue`/`setButtonText`. Used `MCPToolsPluginInterface` to decouple settings tab from main plugin class. Port change uses explicit "Apply" button + confirmation modal rather than auto-apply on change. Deviated: `claude mcp add` command uses `--transport http` (verified against CLI docs), not `--type streamable-http` from design spec. Flags before positional args per CLI convention.                                                                                                                                                              |
| 2026-03-15 | Task 6 | plugin/src/main.ts, plugin/src/main.test.ts, plugin/eslint.config.mts, plugin/src/server.ts, plugin/src/server.test.ts | Wired server and auth into plugin lifecycle. Mocked `McpServer` and `HttpServer` as classes (not `vi.fn().mockImplementation`) to avoid `not a constructor` errors. Added `// @vitest-environment jsdom` for main tests (PluginSettingTab mock needs DOM). `onunload` is sync (void return) per Plugin base class — uses `void this.httpServer.stop()`. Added `plugins: { obsidianmd }` to eslint rule override object to fix plugin resolution across flat config objects. `MCPToolsSettings`/`DEFAULT_SETTINGS` reused from settings.ts (not duplicated). **Deviated:** Live testing revealed `McpServer` only supports one transport at a time — changed `HttpServerConfig.mcpServer` to `createMcpServer` factory function so each session gets its own instance. Design spec assumed single shared instance. |
...
**Summary:** HTTP server with Streamable HTTP transport, Bearer token auth, stateful session management, API key generation/storage, settings tab with connection info, and full plugin lifecycle wiring. All 6 tasks completed, all 14 acceptance criteria passed (including live verification in Obsidian).

**Deviations from Plan:**

- Transport class is `StreamableHTTPServerTransport` (not `NodeStreamableHTTPServerTransport`)
- `claude mcp add` uses `--transport http` (not `--type streamable-http`)
- `McpServer` requires one instance per session — changed from shared instance to `createMcpServer` factory function
- `onunload` is sync (Plugin base class returns void) — server stop is fire-and-forget
- Added `plugins: { obsidianmd }` to eslint config for rule resolution across flat config objects
